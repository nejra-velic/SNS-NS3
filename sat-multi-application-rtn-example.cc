...
//  LogComponentEnable ("PacketSink", LOG_LEVEL_ALL);
  ...
  simulationHelper->SetBeamSet ({12,22,44,45});
  ...
  utUsers.Create (bth.GetNNodesTopology());
  gwUsers.Create (bth.GetNNodesTopology());
  ...
  utCbrUsers.Create (bth.GetNNodesTopology());
  utOnOffUsers.Create (bth.GetNNodesTopology());
  ...
  */
  ...
  s->AddGlobalRtnAppThroughput (SatStatsHelper::OUTPUT_SCATTER_PLOT);

  s->AddGlobalRtnAppDelay (SatStatsHelper::OUTPUT_SCATTER_FILE);
  s->AddGlobalRtnAppDelay (SatStatsHelper::OUTPUT_SCATTER_PLOT);
